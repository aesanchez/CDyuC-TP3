 ...
 *  extracted from:
 *      RTTTL player
 *      By Fábio Pereira (01/09/08)
 *      Joinville - SC - Brasil
 *      www.sctec.com.br
 */
#include <hidef.h> /* for EnableInterrupts macro */
#include "derivative.h" /* include peripheral declarations */
#include "play.h" /* include peripheral declarations */
...
// RTTL music collection (se pueden bajar de la web más ringtones en formato RTTL)
...
// The following array stores the frequencies for the musical notes [Hz]
...
//Función que reproduce toda la lista de canciones, de a una por vez, con una pausa de 2seg
void reproductor (void){
	
	unsigned char song_sel;
	
	for (song_sel=0;song_sel<23;song_sel++)
		      {
...
void play_song(char *song)
{
  ...
  while (*song!=':')            // repeat until find a ':'
  {
    if (*song == 'd')           // if it is the duration setting
    {
      ...
      // if the character is a number, then set the duration
  ...
  while (*song)                 // repeat until the character is null
  {
    ...
    if (current_note<255) 
    	sound(note[temp_octave-4][current_note],calc_duration);
    else
...
//Manejador para la interrupción del CH0 TPM1 que controla la temporización de 1ms
void Tpm1CH0_Handler(void){
	
	TPM1C0V += 4000;                      	// next compare in 1 ms
	if (duration_timer)
		duration_timer--; 					// decrement the timer if > 0
...
//Manejador para la interrupción del CH1 TPM1 que controla el toogle de la salida
